...
export function SpreadsCreativeSpace() {
  ...
  //
  // SINGLE-SESSION (Phase 06, 2026-08-05): shapes are NO LONGER a SCENE-space item — they are edited
  // only in the OBJECTS space (sole writer rtype 10). The former SCENE dual-session (a 2nd rtype-10
  // "retouch" session that existed only to persist shape edits made from here) is RETIRED, so this
  // space mounts ONE session (rtype 6 / SCENE_OWNED_KEYS).
  ...
  // registrant wins). Null the held target → the hook release-saves the scene session + unlocks.
  // Stable (setters only) so the registration effect runs once.
