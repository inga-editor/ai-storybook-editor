...
import { detectMediaQuality, type MediaQuality } from '@/features/editor/components/playable-spread-view/media-quality';
...
export function PlayerViewer({ payload, options, onEvent }: PlayerViewerProps) {
  ...
  // Media rendition quality (ADR-057): parent override wins (sanitized by the
  // embed bridge), otherwise detect once from viewport × DPR. Re-resolves when a
  // new player:init carries a different mediaQuality.
  const mediaQuality = useMemo<MediaQuality>(
    () => opts.mediaQuality ?? detectMediaQuality(),
    [opts.mediaQuality],
  );
